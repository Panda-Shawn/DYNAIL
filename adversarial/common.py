...
def compute_train_stats(
    ...
    pairs = [
        ...
        ("disc_logits", float(th.mean(disc_logits_gen_is_high))),
...
class AdversarialTrainer(base.DemonstrationAlgorithm[types.Transitions]):
    ...
    def __init__(
        ...
        *,
        ...
        q_sas_net: reward_nets.RewardNet = None,
        q_sa_net: reward_nets.RewardNet = None,
        ...
        self.w_p = th.FloatTensor([pos_weight]).to(gen_algo.device)
        ...
        self.q_sas_net = q_sas_net
        if q_sas_net:
            self._q_sas_opt = self._disc_opt_cls(
                self.q_sas_net.parameters(),
                **self._disc_opt_kwargs,
            )

        self.q_sa_net = q_sa_net
        if q_sa_net:
            self._q_sa_opt = self._disc_opt_cls(
                self.q_sa_net.parameters(),
                **self._disc_opt_kwargs,
            )
    ...
    def train_disc(
        ...
        *,
        ...
        with self.logger.accumulate_means("disc"):
            ...
            batch = self._make_noisy_disc_train_batch(
                gen_samples=gen_samples,
                expert_samples=expert_samples,
            )
            disc_logits_tup = self.logits_gen_is_high(
                batch["state"],
                batch["action"],
                batch["next_state"],
                batch["done"],
            ...
            if self.q_sas_net:
                disc_logits = disc_logits_tup[0]
                delta_reward_tup = disc_logits_tup[3]
                disc_reward = delta_reward_tup[0].detach().cpu().numpy()
                delta_reward = delta_reward_tup[1].detach().cpu().numpy()
                log_ratio = delta_reward / delta_reward_tup[2]
                dr_path = os.path.join(self._log_dir, "reward")
                os.makedirs(dr_path, exist_ok=True)
                with open(os.path.join(dr_path, f"delta_reward_record.txt"), "at") as f:
                    f.write(
                        f"Disc step: {self._disc_step}, Disc reward: {disc_reward}\nDelta reward: {delta_reward}, Log ratio: {log_ratio}\n")
            else:
                disc_logits = disc_logits_tup
            ...
            loss.backward()
            th.nn.utils.clip_grad_value_(self._reward_net.parameters(), 1.0)
            self._disc_opt.step()
            self._disc_step += 1

            loss_path = os.path.join(self._log_dir, "loss")
            os.makedirs(loss_path, exist_ok=True)
            with open(os.path.join(loss_path, f"loss_disc.txt"), "at") as f:
                f.write(
                    f"Step: {self._disc_step}, Loss: {loss.detach().cpu().numpy()}\n")
    ...
    def train_q_sas(
        ...
        *,
        ...
        Returns:
            Statistics for discriminator (e.g. loss, accuracy).
        """
        with self.logger.accumulate_means("q_sas"):
            # optionally write TB summaries for collected ops
            write_summaries = self._init_tensorboard and self._global_step % 20 == 0
            ...
            q_sas_logits = disc_logits_tup[1]
            loss = F.binary_cross_entropy_with_logits(
                q_sas_logits,
                batch["labels_gen_is_one"].float(),
                pos_weight=self.w_p,
            ...
            # do gradient step
            self._q_sas_opt.zero_grad()
            loss.backward()
            th.nn.utils.clip_grad_value_(self.q_sas_net.parameters(), 1.0)
            self._q_sas_opt.step()
            self._q_sas_step += 1

            loss_path = os.path.join(self._log_dir, "loss")
            os.makedirs(loss_path, exist_ok=True)
            with open(os.path.join(loss_path, f"loss_q_sas.txt"), "at") as f:
                f.write(
                    f"Step: {self._q_sas_step}, Loss: {loss.detach().cpu().numpy()}\n")
            ...
            for k, v in train_stats.items():
                self.logger.record(k, v)
            self.logger.dump(self._q_sas_step)
            if write_summaries:
                self._summary_writer.add_histogram("disc_logits", q_sas_logits.detach())
    ...
    def train_q_sa(
        ...
        *,
        ...
        Returns:
            Statistics for discriminator (e.g. loss, accuracy).
        """
        with self.logger.accumulate_means("q_sa"):
            # optionally write TB summaries for collected ops
            write_summaries = self._init_tensorboard and self._global_step % 20 == 0
            ...
            # do gradient step
            self._q_sa_opt.zero_grad()
            loss.backward()
            th.nn.utils.clip_grad_value_(self.q_sa_net.parameters(), 1.0)
            self._q_sa_opt.step()
            self._q_sa_step += 1

            loss_path = os.path.join(self._log_dir, "loss")
            os.makedirs(loss_path, exist_ok=True)
            with open(os.path.join(loss_path, f"loss_q_sa.txt"), "at") as f:
                f.write(
                    f"Step: {self._q_sa_step}, Loss: {loss.detach().cpu().numpy()}\n")
            ...
            for k, v in train_stats.items():
                self.logger.record(k, v)
            self.logger.dump(self._q_sa_step)
            if write_summaries:
                self._summary_writer.add_histogram("disc_logits", q_sa_logits.detach())
    ...
    def train(
        ...
        for r in tqdm.tqdm(range(0, n_rounds), desc="round"):
            ...
            if (r+1) % 20 == 0 and r > 0:
                learners_rewards_after_training, _ = evaluate_policy(
                    self.gen_algo, self.venv, 20, return_episode_rewards=True)
                print(f"Round = {r}")
                print(f"Mean = {np.mean(learners_rewards_after_training)}")
                print(f"+/- Std = {np.std(learners_rewards_after_training)}")

                plt.figure(1)
                plt.hist(learners_rewards_after_training)
                fig_path = os.path.join(self._log_dir, "reward")
                os.makedirs(fig_path, exist_ok=True)
                plt.savefig(os.path.join(fig_path, f"reward_round{r}.png"))
                with open(os.path.join(fig_path, f"reward_record.txt"), "at") as f:
                    f.write(
                        f"Episode Rewards: {learners_rewards_after_training}\n")
                plt.close(1)
    ...
    def _make_disc_train_batch(
        ...
        *,
        ...
        with th.no_grad():
            ...
            elif hasattr(self.gen_algo.policy.actor, "action_log_prob"):
                _, log_act_prob_th = self.gen_algo.policy.actor.action_log_prob(
                    obs_th
                )
                log_act_prob = log_act_prob_th.detach().cpu().numpy()
                del log_act_prob_th  # unneeded
    ...
    def _make_noisy_disc_train_batch(
        self,
        *,
        gen_samples: Optional[Mapping] = None,
        expert_samples: Optional[Mapping] = None,
        **kwargs,
        ...
        """
        if expert_samples is None:
            expert_samples = self._next_expert_batch(**kwargs)

        # print("demo_batch_size: ", self.demo_batch_size)
        if gen_samples is None:
            if self._gen_replay_buffer.size() == 0:
                raise RuntimeError(
                    "No generator samples for training. " "Call `train_gen()` first.",
        ...
        dones = np.concatenate([expert_samples["dones"], gen_samples["dones"]])
        labels_gen_is_one = np.concatenate(
            [np.zeros(n_expert, dtype=float), np.ones(n_gen, dtype=float)],
        )

        # Add Guassian noise.
        mean = 0
        std = 1.0
        obs_noise = np.random.normal(mean, std, (obs.shape[0], obs.shape[1]))
        obs += obs_noise
        acts_noise = np.random.normal(mean, std, (acts.shape[0], acts.shape[1]))
        acts += acts_noise
        next_obs_noise = np.random.normal(
            mean, std, (next_obs.shape[0], next_obs.shape[1]))
        next_obs += next_obs_noise
        with th.no_grad():
            obs_th = th.as_tensor(obs, device=self.gen_algo.device)
            acts_th = th.as_tensor(acts, device=self.gen_algo.device)
            log_act_prob = None
